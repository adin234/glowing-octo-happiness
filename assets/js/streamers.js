...
var render_featured_games = function (filter) {
    ...
    render_videos = function (filter, game, lanparty) {
        ...
        filterGame = (typeof game == 'undefined' || game == '' || game == 'all') ? 'all' : $('a.game[data-id=' + game +
                ']').first().attr('data-name'),
            filterGameRegExp = new RegExp(filterGame, 'i'),
            temToComplete = 9,
            source = lanparty && typeof lanparty != 'undefined' ? page_data.lanparty : page_data.streamers,
        ...
        source.forEach(function (item, i) {
            if (typeof item.twitch != 'undefined') {
                if (typeof filter != 'undefined' && item.twitch.channel.status && !~item.twitch.channel.status.search(
                ...
                item.live = 'live';
                item.link = origin + (lanparty && typeof lanparty != 'undefined' ? 'lanparty_stream_multi/#/' +
                    item.id : 'gamer_stream/?user=' + item.user_id + '/#!/' + item.id);
                ...
                if (typeof filter != 'undefined' && !~hitboxData.media_status.search(filterRegExp) && !~item.user
                ...
                if (typeof filter != 'undefined' && !~item.username.search(filterRegExp)) {
