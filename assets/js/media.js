...
var update_videos = function (videos, append, initial) {
    ...
    for (var k = start; k < start + 20; k++) {
        ...
        if (filterTags && (typeof item.snippet.meta == 'undefined' || typeof item.snippet.meta.tags == 'undefined' ||
...
var filterGame = function (filterString) {
    ...
    page_data.playlists.forEach(function (item) {
        if (typeof item.snippet.meta != 'undefined' && (~item.snippet.meta.tags.indexOf('anytv_' +
                filterString) || ~item.snippet.meta.tags.indexOf(filterString))) {
            playlists.push(item);
        }
    });
...
function formatDate(date) {
    ...
    return formattedDAte = now.getDate() + "-" + months[now.getMonth()] + "-" + now.getFullYear() + ' ' + now.getHours() +
        ':' + now.getMinutes();
}
